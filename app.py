...
def main():
    ...
    sheet_options = ["All"] + sheet_names
    selected_sheet = st.sidebar.selectbox("Select Building/Location:", sheet_options)
    
    # Get selected data (optionally combine all sheets)
    if selected_sheet == "All":
        df = pd.concat(
            [data.assign(**{"Building/Location": sheet_name}) for sheet_name, data in ledger_data.items()],
            ignore_index=True
        )
        header_label = "All Buildings/Locations"
        download_prefix = "all_locations"
    else:
        df = ledger_data[selected_sheet].copy()
        header_label = selected_sheet
        download_prefix = selected_sheet
    
    st.header(f"📊 {header_label}")
    ...
    st.download_button(
        ...
        data=csv,
        file_name=f"{download_prefix}_filtered_{datetime.now().strftime('%Y%m%d')}.csv",
        mime="text/csv"
    )
